...
//            .filter({($0.definition?.tartan.colorSet.count)! > 5 })
            .filter({($0.definition?.tartan.colors.count)! < 8 })
    ...
    func analyzePublicContents() { //
        ...
        _ = Array(patternCollection)
            .map({p in patterns[p] = publicContents.filter({$0.definition?.tartan.colorSetCompact.sortString == p}).count
            })
        _ = patterns
            .filter({$0.value > 1})
            .sorted(by: {$1.key.count < $0.key.count}).map({print("\($0.key): \($0.value) ")})
//clrs: \($0.key.reduce(0){ar, clr in clr > ar ? clr : ar})
...
class TartanLibrary: NSObject {
    ...
    func parseInput() {
        var colorCodeLibrarySet: Set<String> = []
        ...
        { (dict, content) -> [String: (tartan: Tartan, info: [String:Any])] in
            ...
            let sizes = zones.reduce([]) {
                (result, zone) in result + [Int(zone
                    ...
                    .joined(separator: ""))]}
            //
            if let availableSizes = sizes as? [Int] { //todo remove trailing comma in TartanDefService.definitionStrings
                ...
                //                maintain library's set of colorCodes
                colorCodeLibrarySet = colorCodeLibrarySet.union(Set(colorStrings))
                ...
                if let availableMap = colorStrings.reduce([], { ar, el in ar + [palet.colorMap[el]?.0] }) as? [Int] {
                    ...
                    tartanInfo["hatseflats"] = 1234
                    //tartanInfo["sortValue"] = entry.tartan.colorSet.reduce("") { ar, el in ar + String(el) }
                    tartanInfo["sortValue"] = entry.tartan.colorSet
                    entry.info = tartanInfo
                    
                    //while title exists, postfix increasing version
                    var uniqueTitle = content.title
                    while dict[uniqueTitle] != nil {
                        uniqueTitle = uniqueTitle + ".x"
                        //                        print("uniqueTitle: \(uniqueTitle)")
                    }
...
struct ParsedTextFile {
    ...
    //    var t:(title: String, definition: String)
...
//only still there for anagrams
//    public let content: [Tartan] //library.tartans()

//    func entryFor(colorSet: Set<Int>) -> [String: Definition] { //board.colorSet
//        return [:]
//    }
